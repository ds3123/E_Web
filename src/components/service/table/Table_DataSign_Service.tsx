...
const Table_DataSign_Service : FC< Sign > = ( { isFetching , search , total_data_sum } ) => {
    ...
    return <div className = "flex justify-center relative top-20">
    
                { /* 下載中 icon */ } 
                 { isFetching &&  <FaSpinner className = "animate-spin text-gray-500 relative top-20" size = { 32 }  />  }


                 { ( !search && total_data_sum === 0 ) && 
                      <p className = "flex items-center justify-center bg-red-500 px-5 py-3 text-white text-xl rounded-full">
